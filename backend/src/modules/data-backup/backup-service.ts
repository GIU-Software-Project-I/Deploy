...
export class BackupService {
    ...
    async createBackup(options: BackupOptions = {}): Promise<BackupMetadata> {
        ...
        // Generate data-backup filename with timestamp
        ...
        try {
            ...
            // Get data-backup directory size and collection stats
            ...
            // Write manifest.json with data-backup metadata
    ...
    private async convertBackupToJson(backupPath: string): Promise<void> {
        try {
            // Get all directories in the data-backup path (these are database names)
    ...
    async deleteBackup(filename: string): Promise<void> {
        try {
            ...
            if (!backupPath.startsWith(this.backupDir)) {
                throw new Error('Invalid data-backup path');
            }
