...
PositionSchema.pre('save', async function (next) {
    try {
        ...
        // If db is not available, skip hierarchy check
        if (!db) {
            console.warn('Position Pre-Save: Database connection not available. Skipping hierarchy check.');
            return next();
        }

        // Ensure Department is registered
        // If not found, skip hierarchy check
        ...
        // Resolve department head for reporting hierarchy
