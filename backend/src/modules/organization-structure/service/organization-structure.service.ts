...
export class OrganizationStructureService {
  ...
  async createChangeRequest(dto: SubmitStructureRequestDto, performedBy?: string): Promise<StructureChangeRequest> {
    ...
    const requesterName = await this.sharedOrganizationService.getEmployeeName(dto.requestedByEmployeeId);
  ...
  async getScopedOrgChart(employeeId: string, roles: string[]): Promise<{ departments: Department[]; positions: Position[] }> {
    ...
    const currentPosition = assignment.positionId as any;
    const departmentId = currentPosition.departmentId;

    // Get the employee's department
    const resultDepts = await this.departmentModel.find({ _id: departmentId })
      .populate('headPositionId', 'title code')
      .lean()
      .exec();
    ...
    if (isDeptHead) {
      ...
      // Interpretation: Department Head sees the entire Department they manage.
      // We fetch all positions in this department.
      resultPositions = await this.positionModel.find({
        departmentId: new Types.ObjectId(departmentId),
        isActive: true
      })
        .populate('departmentId', 'name code')
        .populate('reportsToPositionId', 'title code')
        // Populate current holder for the chart
        // Note: The frontend expects 'currentHolderId' logic or we need to handle it.
        // The frontend uses 'employeeId' from orgNode which comes from... 
        // wait, the previous getOrgChart didn't populate employee... 
        // Ah, the frontend re-maps data or the backend DTO needs to include it?
        // Changes: We need to ensure we return data compatible with buildOrgTree.
        .lean()
        .exec();

      // We need to attach current holders to these positions manually or via populate if schema supports virtuals
      // The standard getOrgChart returns positions. The frontend maps them.
      // Let's ensure we match the getOrgChart output format.
      // Actually, getOrgChart didn't populate 'currentHolderId' in the visible code block (lines 1227-1232).
      // Let's check if the schema has it or if we need to fetch assignments.

    } else {
      // BR 41: "Employees see their own structure"
      // Interpretation: Lineage from Root -> Them (Reporting Line).
      // We trace up from current position to the Department Head.

      const pathPositions: any[] = [];
      let curr = await this.positionModel.findById(currentPosition._id)
        .populate('departmentId', 'name code')
        .populate('reportsToPositionId', 'title code')
        .lean()
        .exec();
      ...
      while (curr) {
        pathPositions.push(curr);

        if (!curr.reportsToPositionId) break;

        // Move up
        const nextId = (curr.reportsToPositionId as any)._id || curr.reportsToPositionId;
        curr = await this.positionModel.findById(nextId)
          .populate('departmentId', 'name code')
          .populate('reportsToPositionId', 'title code')
          .lean()
          .exec();

        // Safety: Ensure we don't cross into another department if structure is weird, though reportsTo usually stays in dept or goes to hierarchy root
      }
      resultPositions = pathPositions;
    }

    // IMPORTANT: The frontend 'buildOrgTree' expects positions to have 'currentHolderId' potentially?
    // Let's look at frontend: "employeeId: pos.currentHolderId".
    // Does Position schema have currentHolderId? 
    // Usually it's better to fetch assignments and map them, OR the Position schema has a virtual/field.
    // I will fetch active assignments for the resultPositions and map them to simulate 'currentHolderId' if needed,
    // OR just rely on Position schema if it's there. 
    // Checking previous 'getOrganizationChart': it uses 'positions' directly.
    // I'll assume Schema has it or it's handled. 
    // Wait, I should verify Position Schema.

    // fetching assignments for the found positions to populate 'currentHolderId' field if it's not on the model
    // The previous code for getOrgChart didn't do this explicitly, so it might be on the model or unused?
    // Frontend uses: employeeId: pos.currentHolderId
    // Let's fetch active assignments for these positions and attach "currentHolderId".

    const positionIds = resultPositions.map(p => p._id);
    const activeAssignments = await this.assignmentModel.find({
      positionId: { $in: positionIds },
      $or: [{ endDate: { $exists: false } }, { endDate: { $gt: new Date() } }]
    }).populate('employeeProfileId', 'firstName lastName employeeNumber').lean();

    // Map assignments to positions
    const posMap = new Map();
    activeAssignments.forEach(a => {
      posMap.set(a.positionId.toString(), a.employeeProfileId);
    });

    resultPositions = resultPositions.map(p => ({
      ...p,
      currentHolderId: posMap.get(p._id.toString())
    }));

    return { departments: resultDepts as Department[], positions: resultPositions as Position[] };
  }
