...
import { SharedPayrollService } from '../../../integration/services/shared-payroll.service';
import { stat } from 'fs';
import {
    claims
...
export class PayrollExecutionService {
    ...
    async listSigningBonuses(status?: string) {
        ...
        if (db && bonuses.length > 0) {
            ...
            const employeeMap: any = new Map(employees.map(e => [
                e._id.toString(),
                {
            ...
            return bonuses.map(b => {
                const empInfo = employeeMap.get(b.employeeId?.toString());
                return {
                    ...b,
