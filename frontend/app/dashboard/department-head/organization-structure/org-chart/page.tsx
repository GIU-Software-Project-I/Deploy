 ...
 * BR 41: Access must be role-based, where Direct Managers see their team only
 */
...
export default function DepartmentHeadOrgChartPage() {
    const { user } = useAuth();
    ...
    const fetchOrgChart = async () => {
        try {
            ...
            // The backend service should handle filtering based on role/permissions context
            // For department head, we might want to pass a specific flag or rely on backend session
            const res = await organizationStructureService.getOrgChart();

            // Filter the tree to only show relevant sub-tree if needed, 
            // though ideally backend handles this for better security/performance.
            // For now, assuming backend returns all visible nodes.
            // We will perform a client-side filter to only show departments/positions relevant to the manager
            // A simple heuristic: if we can identify the manager's department or position.
    ...
    const renderTreeNode = (node: OrgNode, depth: number = 0) => {
        ...
        return (
            <div key={node._id} className="select-none">
                <div
                    className={`flex items-center gap-2 py-2 px-3 rounded-lg hover:bg-muted/50 transition-colors ${depth > 0 ? 'ml-6' : ''
                        }`}
    ...
    if (loading) {
        return (
            <RoleGuard allowedRoles={[SystemRole.DEPARTMENT_HEAD, SystemRole.SYSTEM_ADMIN, SystemRole.HR_ADMIN]}>
                        ...
                        <div className="flex items-center gap-4">
                            ...
                            <div>
                                ...
                                <p className="text-muted-foreground text-sm mt-1">
                                    View departmental hierarchy and reporting lines
                                </p>
                            </div>
                        </div>
                    </div>
                    ...
                    <div className="bg-card border border-border rounded-xl">
                        {orgData.length === 0 ? (
                            <div className="p-12 text-center">
                                ...
                                <h3 className="font-medium text-foreground">No Structure Data</h3>
                                <p className="text-muted-foreground text-sm mt-1">
